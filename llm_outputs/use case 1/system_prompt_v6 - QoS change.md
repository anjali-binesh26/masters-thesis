...
| Property | Value |
|----------|-------|
| Configuration | `<config_file>.<path>` or `N/A` |
| Runtime API | `<message>.<parameter>` or `N/A` |
| Required Inputs | Required identifiers or inputs for runtime modification (e.g., `imsi`, `pdu_session_id`) |
| Constraints | Valid values, units (if applicable), and brief operational meaning |
| Related Parameters | Parameters explicitly documented as commonly modified together |
| Purpose | One concise sentence describing what the parameter controls |
...
| Constraints | Integer 1-254. Standardized QoS Identifier defining predefined latency, reliability and priority characteristics. |
